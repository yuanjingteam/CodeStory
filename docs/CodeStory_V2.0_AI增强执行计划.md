...
| 事件 | 允许提升 | 允许下降 | 说明 |
| --- | :---: | :---: | --- |
| 选择题答对（规则判定） | ✅ | — | 对错由 `exercise.service.ts:218-228` 的选项比对决定，AI 不参与 |
| 选择题答错（规则判定） | — | ❌ | 只记录错题，不下调既有掌握度 |
| 代码题 AI 评阅通过且置信度达标 | ✅ | — | 沿用 V1 的 `calculateAiReviewedFinalScore` |
| 代码题 AI 评阅未通过 | — | ❌ | 维持现状，不下调；进入错题集 |
| 规则结论与 AI 结论冲突 / 置信度低于阈值 | ❌ | ❌ | 保留既有掌握度，只记入错题集 |
| 状态机 `REVIEW` 节点（hint_level=3 仍未通过） | — | ❌ | 阶段 4；直接给出参考答案与解析结束本轮，不下调掌握度 |

> **2026-08-05 修订**：人工复核队列已整体移除，本表原有的「人工复核改判为掌握 / 未掌握」两行随之删除。**掌握度不再有任何下降入口**。
>
> **2026-08-07 修订**：随之删除「需人工复核」一列——移除后每行恒为「否」，该列不再承载信息。
...
- **幂等：**
  ...
  - **审核状态流转用条件更新**（`WHERE review_status = 'draft'`），避免并发采用/拒绝相互覆盖。~~复核记录以 `submission_fingerprint` 唯一约束按提交快照去重。~~ **2026-08-05 移除**：复核记录已不存在。
