...
Compose 不包含 Nginx 容器。正式部署前先按实际域名修改仓库中的
`nginx/conf.d/default.conf` 并配置 HTTPS，但此时不要安装或重载配置，避免在
数据库迁移和业务回归完成前意外开放公网入口。Nginx 的安装与重载放在第 6 节执行。
...
```bash
echo "69E2517415782F70935D1D5C12C64A70217FC92A240ED5B5F402FE014780628B  backups/codestory_db_migrated_20260727.dump" \
  | sha256sum --check
```

校验失败时立即停止，不得继续恢复。uploads 归档也应使用归档交付方提供的
SHA-256 校验值完成校验后再解压。
...
迁移目录会随版本继续增加，应以 `backend/prisma/migrations` 的实际内容为准。
生产归档内已有的基础迁移包括：
...
（内部修订号 `grounded-v3.1-boundary`）。复查脚本位于 backend，使用同一个临时
容器连续执行，确保前序命令生成的报告可供后续命令读取：
...
可通过 SSH 端口转发访问仅绑定在 `127.0.0.1` 的 frontend 和 backend 完成回归。

全部回归通过后，再安装并启用宿主机 Nginx 配置：
...
如果服务器已有默认站点或其他监听相同域名的配置，应先处理冲突；不要在
`nginx -t` 失败时重载。
