...
export default function AuthPage() {
  ...
  const mode = params.mode as 'login' | 'register' | 'forgot-password' | 'reset-password';

  const getPageTitle = () => {
    switch (mode) {
      case 'login':
        return { title: 'WELCOME BACK!', subtitle: '登录你的账号，继续学习编程', label: '登录', color: 'bg-purple-500' };
      case 'register':
        return { title: 'CREATE ACCOUNT', subtitle: '加入 Code Learns，开启你的编程之旅', label: '注册', color: 'bg-green-500' };
      case 'forgot-password':
        return { title: 'FORGOT PASSWORD', subtitle: '输入邮箱，找回你的密码', label: '找回密码', color: 'bg-yellow-500' };
      default:
        return { title: 'WELCOME BACK!', subtitle: '登录你的账号，继续学习编程', label: '登录', color: 'bg-purple-500' };
    }
  };
